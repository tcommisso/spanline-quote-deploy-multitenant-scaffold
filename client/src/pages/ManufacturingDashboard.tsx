    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <KPICard
          ...
          icon={Package}
          detail={`${summary?.componentOrders ?? 0} component · ${summary?.flashingOrders ?? 0} flashing · ${summary?.transitionOrders ?? 0} uploaded`}
        />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <QuickLink href="/manufacturing/orders" icon={ClipboardList} title="Orders" description="Manage order status, target dates, and production tasks" />
...
function KPICard({ label, value, icon: Icon, color, detail }: { label: string; value: number; icon: any; color?: string; detail?: string }) {
  return (
    <div className="bg-card border rounded-lg p-4">
      ...
      {detail && <p className="mt-1 text-[11px] leading-tight text-muted-foreground">{detail}</p>}
    </div>
  );
}
