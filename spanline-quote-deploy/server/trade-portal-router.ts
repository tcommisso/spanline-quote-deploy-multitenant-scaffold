...
export const tradePortalRouter = router({
  ...
  requestMagicLink: publicTradePortalProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const magicLinkUrl = buildTrustedAppUrl(
        ctx.req,
      ...
      try {
        ...
        if (resendApiKey) {
          ...
          const resp = await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
              from: `${ENV.emailSenderName || "Altaspan"} <${ENV.emailSenderAddress || "support@commissogroup.au"}>`,
          ...
          if (!resp.ok) {
            const errBody = await resp.text().catch(() => "Unknown API error");
            await logNotification(
              ...
              "failed",
              `Resend API ${resp.status}: ${errBody}`
            );
          } else {
            await logNotification(
              { settingKey: "trade_portal.magic_link", channel: "email", recipientType: "trade", recipientId: input.email, title: "Trade Portal Login Link" },
