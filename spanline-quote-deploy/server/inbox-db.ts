...
export async function getInboxMessageById(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  const [msg] = await db.select().from(inboxMessages).where(eq(inboxMessages.id, id)).limit(1);
  return msg || null;
}
...
export async function updateInboxMessage(id: number, data: Partial<InsertInboxMessage>) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  await db.update(inboxMessages).set(data).where(eq(inboxMessages.id, id));
}

export async function markAsRead(ids: number[]) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  await db.update(inboxMessages).set({ isRead: true }).where(inArray(inboxMessages.id, ids));
}

export async function markAllAsRead() {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  await db.update(inboxMessages).set({ isRead: true }).where(eq(inboxMessages.isRead, false));
}

export async function markAsUnread(ids: number[]) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  await db.update(inboxMessages).set({ isRead: false }).where(inArray(inboxMessages.id, ids));
}

export async function toggleStar(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  const [msg] = await db.select({ isStarred: inboxMessages.isStarred }).from(inboxMessages).where(eq(inboxMessages.id, id)).limit(1);
  if (!msg) throw new Error("Message not found");
  await db.update(inboxMessages).set({ isStarred: !msg.isStarred }).where(eq(inboxMessages.id, id));
  return !msg.isStarred;
}

export async function assignMessage(id: number, assignedToId: number | null, assignedToName: string | null) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
...
export async function matchEmailToClient(fromEmail: string): Promise<{
  ...
  const [portalMatch] = await db
    ...
    .from(portalAccess)
    .where(and(eq(portalAccess.clientEmail, email), eq(portalAccess.isActive, true)))
    .limit(1);
  ...
  if (portalMatch) {
    ...
    const [job] = await db
      ...
      .where(eq(constructionJobs.id, portalMatch.jobId))
      .limit(1);
    return {
  ...
  const [leadMatch] = await db
    ...
    .from(crmLeads)
    .where(eq(crmLeads.contactEmail, email))
  ...
  if (leadMatch) {
    ...
    const [job] = await db
      ...
      .from(constructionJobs)
      .where(eq(constructionJobs.leadId, leadMatch.id))
      .limit(1);
    return {
...
export async function listInboxAddresses(activeOnly = true) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  const conditions: any[] = [];
...
export async function getInboxAddressByEmail(email: string): Promise<InboxAddress | null> {
  const db = await getDb();
  if (!db) return null;
  ...
  const [row] = await db
    .select()
    .from(inboxAddresses)
    .where(eq(inboxAddresses.address, normalized))
    .limit(1);
  return row || null;
}
...
export async function updateInboxAddress(id: number, data: Partial<InsertInboxAddress>) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  if (data.address) data.address = data.address.toLowerCase().trim();
  await db.update(inboxAddresses).set(data).where(eq(inboxAddresses.id, id));
}

export async function deleteInboxAddress(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  await db.delete(inboxAddresses).where(eq(inboxAddresses.id, id));
}
 ...
 */
export async function matchReceivingAddress(toAddresses: string[]): Promise<InboxAddress | null> {
  const db = await getDb();
  if (!db) return null;
...
export async function bulkDeleteMessages(ids: number[]) {
  ...
  await db.delete(inboxMessageTags).where(inArray(inboxMessageTags.messageId, ids));
  // Delete messages
  await db.delete(inboxMessages).where(inArray(inboxMessages.id, ids));
}

export async function bulkAssignMessages(ids: number[], assignedToId: number | null, assignedToName: string | null) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  await db.update(inboxMessages).set({
    assignedToId,
...
export async function bulkAddTag(ids: number[], tagId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
