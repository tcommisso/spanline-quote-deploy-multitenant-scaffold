...
import { nylasGrants, crmAppointments, users } from "../drizzle/schema";
...
export const nylasRouter = router({
  ...
  getMyGrants: protectedProcedure.query(async ({ ctx }) => {
    ...
    const grants = await db
      .select()
      .from(nylasGrants)
      .where(and(eq(nylasGrants.userId, ctx.user.id), eq(nylasGrants.status, "active")));
    return grants;
  }),
  ...
  getGrant: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [grant] = await db
      .select()
      .from(nylasGrants)
      .where(eq(nylasGrants.userId, ctx.user.id))
      .limit(1);
    return grant || null;
  }),
   ...
   */
  listGrants: adminProcedure.query(async () => {
    const db = await getDb();
    if (!db) return [];
    const grants = await db
      .select({
  ...
  getAuthUrl: protectedProcedure
    ...
    .query(({ ctx, input }) => {
      ...
      const url = buildAuthUrl(input.redirectUri, state, {
  ...
  exchangeCode: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const result = await exchangeCodeForGrant(input.code, input.redirectUri);

      // Check if this grant already exists (same grantId or same email for this user)
      const [existingByGrant] = await db
        .select()
        .from(nylasGrants)
        .where(and(eq(nylasGrants.userId, ctx.user.id), eq(nylasGrants.grantId, result.grant_id)))
        .limit(1);
      ...
      if (existingByGrant) {
        ...
        const [existingByEmail] = await db
          .select()
          .from(nylasGrants)
          .where(and(eq(nylasGrants.userId, ctx.user.id), eq(nylasGrants.email, result.email)))
          .limit(1);
  ...
  disconnect: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input?.grantId) {
        ...
        await db.update(nylasGrants)
          .set({ status: "revoked" })
          .where(and(eq(nylasGrants.id, input.grantId), eq(nylasGrants.userId, ctx.user.id)));
      } else {
  ...
  listCalendars: protectedProcedure.query(async ({ ctx }) => {
    ...
    try {
      const calendars = await listCalendars(grant.grantId);
  ...
  deleteAppointment: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [existing] = await db
        .select()
        .from(crmAppointments)
        .where(eq(crmAppointments.id, input.id))
        .limit(1);
      ...
      if (existing.nylasEventId) {
        ...
        if (grant) {
          try {
            await deleteEvent(grant.grantId, existing.nylasEventId);
          } catch (err: any) {
      ...
      await db.delete(crmAppointments).where(eq(crmAppointments.id, input.id));
