...
export async function scrapeAndStoreT1CloudDas(options?: {
  ...
  const targetConfigs = options?.councils
    ? T1CLOUD_COUNCILS.filter(c => options.councils!.includes(c.name))
    : T1CLOUD_COUNCILS;
    ...
    try {
      ...
      for (const da of scrapedDas) {
        try {
          ...
          const competitorMatch = await matchCompetitor(da.applicantName);
 ...
 */
export async function reMatchCompetitors(): Promise<{ matched: number; total: number }> {
  const db = await getDb();
  ...
  const das = await db.select({
    ...
    .from(nswDaApplications)
    .where(sql`${nswDaApplications.applicantName} IS NOT NULL`);
  ...
  for (const da of das) {
    ...
    await db.update(nswDaApplications)
      ...
      .where(eq(nswDaApplications.id, da.id));
