...
export async function pollDaApplications(): Promise<{
  newCount: number;
  updatedCount: number;
  removedCount: number;
  totalFetched: number;
  ...
  try {
    ...
    for (const feature of features) {
      ...
      if (!existingRecord) {
        ...
        if (inserted) {
          await dispatchWebhooks(db, inserted.id, attrs, "new_da");
        }
        ...
        await db.update(daTrackerApplications)
          ...
          .where(eq(daTrackerApplications.id, existingRecord.id));
        updatedCount++;

        await dispatchWebhooks(db, existingRecord.id, attrs, "updated_da");
      } else {
...
async function dispatchWebhooks(
  ...
  const subscriptions = await db.select()
    ...
    .where(eq(daTrackerWebhookSubscriptions.active, true));
...
export async function processWebhookDeliveries(): Promise<{ delivered: number; failed: number }> {
  ...
  const pending = await db.select()
    ...
    .where(eq(daTrackerWebhookDeliveries.status, "pending"))
    .limit(100);

  for (const delivery of pending) {
    const [sub] = await db.select()
      .from(daTrackerWebhookSubscriptions)
      .where(eq(daTrackerWebhookSubscriptions.id, delivery.subscriptionId))
      .limit(1);
