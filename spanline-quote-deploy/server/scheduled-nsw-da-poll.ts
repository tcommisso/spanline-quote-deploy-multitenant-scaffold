...
export function registerScheduledNswDaPoll(app: Express) {
  app.post("/api/scheduled/nsw-da-poll", async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.log("[NswDaPoll] Starting poll of NSW Planning Portal for all target councils...");
      const pollResult = await pollNswDaApplications();
      console.log(`[NswDaPoll] Poll complete. New: ${pollResult.totalNew}, Updated: ${pollResult.totalUpdated}, Relevant: ${pollResult.totalRelevant}`);

      for (const r of pollResult.results) {
        if (r.errors.length > 0) {
          console.warn(`[NswDaPoll] ${r.council}: ${r.errors.join(", ")}`);
        } else {
          console.log(`[NswDaPoll] ${r.council}: fetched=${r.totalFetched}, new=${r.newCount}, relevant=${r.relevantCount}`);
        }
      }

      // Phase 2: Scrape T1Cloud portals for builder/applicant names (QPRC + Wagga)
      let scrapeResult;
      try {
        console.log("[NswDaPoll] Starting T1Cloud scrape for builder names...");
        scrapeResult = await scrapeAndStoreT1CloudDas();
        console.log(`[NswDaPoll] T1Cloud scrape complete. New: ${scrapeResult.totalNew}, Updated: ${scrapeResult.totalUpdated}, Competitors: ${scrapeResult.totalCompetitorMatches}`);
      } catch (scrapeErr: any) {
        console.error("[NswDaPoll] T1Cloud scrape error (non-fatal):", scrapeErr.message);
      }

      // Notify owner if new relevant DAs found
      if (pollResult.totalRelevant > 0) {
        try {
          const title = `🏗️ ${pollResult.totalRelevant} new relevant NSW DA${pollResult.totalRelevant > 1 ? "s" : ""} detected`;
          ...
          console.error("[NswDaPoll] Notification error (non-fatal):", notifyErr.message);
        }
      }
  ...
  app.post("/api/scheduled/nsw-competitor-digest", async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.log("[NswDigest] Generating weekly NSW competitor digest...");

      // Get recent relevant DAs from the past 7 days
      const recentDas = await getRecentRelevantNswDas(7);

      if (recentDas.length === 0) {
        console.log("[NswDigest] No new relevant DAs in the past week. Skipping digest.");
        return res.json({ ok: true, skipped: "no_new_das", duration: Date.now() - startTime });
      }
      ...
      if (recipients.length === 0) {
        console.log("[NswDigest] No admin recipients found. Skipping.");
        return res.json({ ok: true, skipped: "no_recipients", duration: Date.now() - startTime });
      }
      ...
      console.log(`[NswDigest] Digest sent to ${sent}/${recipients.length} recipients.`);
