 ...
 * Handles Resend email.received events for the Shared Inbox.
...
async function sendAutoReply(params: {
  ...
  const fromDomain = await getSetting("receiving_domain") || "resend.dev";
  ...
  try {
    ...
    if (result.success) {
      await updateInboxMessage(params.inboxMessageId, { autoReplySent: true });
...
export function registerInboxWebhooks(app: Express) {
  ...
  app.post("/api/resend/inbound", async (req: Request, res: Response) => {
    try {
      ...
      const match = await matchEmailToClient(fromAddress);

      // Match receiving address for auto-assignment rules
      const toArray = Array.isArray(toAddresses) ? toAddresses : [toAddresses];
      const addressRule = await matchReceivingAddress(toArray);
      ...
      if (match?.matchedJobId) {
        ...
        if (activity) {
          ...
          await updateInboxMessage(inboxMsgId, { activityId });
        }
      }
