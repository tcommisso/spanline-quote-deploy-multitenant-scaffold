...
export async function runClientDaMatching(options?: {
  ...
  const watchlist = await db.select().from(daCompetitorWatchlist).where(eq(daCompetitorWatchlist.active, true));
  ...
  const leadsQuery = options?.leadIds
    ? db.select({
        id: crmLeads.id,
      ...
      }).from(crmLeads).where(inArray(crmLeads.id, options.leadIds))
    : db.select({
        id: crmLeads.id,
        address: crmLeads.contactAddress,
        suburb: crmLeads.suburb,
    ...
    try {
      ...
      if (!options?.forceRefresh) {
        const existing = await db.select({ id: clientDas.id })
          .from(clientDas)
          .where(eq(clientDas.leadId, lead.id))
          .limit(1);
      ...
      for (const da of daResults) {
        ...
        if (existingMatch.length === 0) {
          ...
          await db.update(clientDas)
            ...
            .where(eq(clientDas.id, existingMatch[0].id));
          matched++;
        }
      }
    } catch (err: any) {
  ...
  for (const quote of quotesWithAddress) {
    try {
      ...
      if (!options?.forceRefresh) {
        const existing = await db.select({ id: clientDas.id })
          .from(clientDas)
          .where(eq(clientDas.quoteId, quote.id))
          .limit(1);
        if (existing.length > 0) { skipped++; continue; }
      }
