...
export const signwellRouter = router({
  ...
  sendForSignature: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const templateSetting = await db.getMasterDataValue("signwell", "template_id");
  ...
  getStatus: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        const doc = await signwell.getDocument(quote.signwellDocumentId);
  ...
  downloadSignedPdf: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const pdfBuffer = await signwell.downloadSignedPdf(quote.signwellDocumentId);
  ...
  cancel: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await signwell.cancelDocument(quote.signwellDocumentId);
      await db.updateQuote(input.quoteId, {
  ...
  webhook: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (eventType === "document_completed") {
        ...
        try {
          const pdfBuffer = await signwell.downloadSignedPdf(doc.id);
