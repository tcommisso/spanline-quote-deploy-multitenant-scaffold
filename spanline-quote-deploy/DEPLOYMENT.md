...
# Apply the schema using Drizzle Kit or import the SQL dump
# Option A: Use Drizzle Kit (requires DATABASE_URL in .env)
pnpm drizzle-kit push
# Option B: Import the SQL dump directly
mysql -u <user> -p <database> < schema-dump.sql
...
|----------|-------------|---------|
| `DATABASE_URL` | MySQL connection string | `mysql://user:pass@host:3306/dbname?ssl={"rejectUnauthorized":true}` |
...
### Option A: Drizzle Kit Push (Recommended for fresh setup)
...
A `schema-dump.sql` file is included in this package. Import it directly:
...
These platforms auto-detect Node.js apps. Ensure:
1. `pnpm build` runs as the build command
2. `node dist/index.js` is the start command
3. All environment variables are configured in the platform dashboard
...
The server responds to requests at the root URL. For load balancer health checks, use:
```
GET / → 200 OK (serves the frontend)
```
...
4. **Database migrations** are managed via `webdev_execute_sql` in the Manus platform. For external deployment, use `drizzle-kit push` or the included SQL dump.
