...
export const calendarViewsRouter = router({
  ...
  getViewMembers: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const members = await db
        ...
        .where(and(eq(calendarViewMembers.tenantId, ctx.tenant!.id), eq(calendarViewMembers.viewType, input.viewType)))
  ...
  addMember: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [existing] = await db
        ...
        .from(calendarViewMembers)
        .where(and(
          eq(calendarViewMembers.tenantId, ctx.tenant!.id),
  ...
  removeMember: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new Error("Database unavailable");
      await db.delete(calendarViewMembers).where(and(eq(calendarViewMembers.tenantId, ctx.tenant!.id), eq(calendarViewMembers.id, input.id)));
      return { success: true };
    }),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (let i = 0; i < input.memberIds.length; i++) {
        await db.update(calendarViewMembers)
          .set({ sortOrder: i })
          .where(and(eq(calendarViewMembers.tenantId, ctx.tenant!.id), eq(calendarViewMembers.id, input.memberIds[i])));
      }
      return { success: true };
    }),
  ...
  getAvailability: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const scheduleBlocks = await db
        ...
        .where(and(eq(userScheduleBlocks.tenantId, ctx.tenant!.id), inArray(userScheduleBlocks.userId, userIds)));
      ...
      const timeOffEntries = await db
        ...
        .from(userTimeOff)
        .where(and(
          eq(userTimeOff.tenantId, ctx.tenant!.id),
  ...
  getMySelections: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const rows = await db
        ...
        .where(and(
          eq(userCalendarSelections.tenantId, ctx.tenant!.id),
  ...
  rescheduleCalendarEvent: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [grant] = await db
        ...
        .where(and(
          eq(nylasGrants.tenantId, ctx.tenant!.id),
          eq(nylasGrants.grantId, input.grantId),
          eq(nylasGrants.status, "active")
        ))
        .limit(1);
      if (!grant) throw new Error("Calendar grant not found");
  ...
  rescheduleJobAssignment: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [job] = await db
        ...
        .where(and(eq(constructionJobs.tenantId, ctx.tenant!.id), eq(constructionJobs.id, input.jobId)))
        .limit(1);
