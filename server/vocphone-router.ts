...
export const vocphoneRouter = router({
  // ─── SMS Numbers ──────────────────────────────────────────────────────────
  getSmsNumbers: protectedProcedure.query(async ({ ctx }) => {
    try {
      return await vocphone.getSmsNumbers(ctx.tenant!.id);
    } catch (e: any) {
      return { total: 0, list: [] };
    }
  }),
  ...
  getLeadMessages: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const messages = await db
        .select()
        .from(smsMessages)
        .where(and(eq(smsMessages.tenantId, ctx.tenant!.id), eq(smsMessages.leadId, input.leadId)))
  ...
  getLeadCalls: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const calls = await db
        .select()
        .from(callLogs)
        .where(and(eq(callLogs.tenantId, ctx.tenant!.id), eq(callLogs.leadId, input.leadId)))
  ...
  getLeadTimeline: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const [messages, calls] = await Promise.all([
        db.select().from(smsMessages).where(and(eq(smsMessages.tenantId, ctx.tenant!.id), eq(smsMessages.leadId, input.leadId))),
        db.select().from(callLogs).where(and(eq(callLogs.tenantId, ctx.tenant!.id), eq(callLogs.leadId, input.leadId))),
      ]);
  ...
  templates: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      ...
      return db.select().from(smsTemplates)
        .where(eq(smsTemplates.tenantId, ctx.tenant!.id))
    ...
    upsert: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.id) {
          ...
          }).where(and(eq(smsTemplates.tenantId, ctx.tenant!.id), eq(smsTemplates.id, input.id)));
          return { id: input.id };
        } else {
    ...
    delete: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const db = (await getDb())!;
        await db.delete(smsTemplates).where(and(eq(smsTemplates.tenantId, ctx.tenant!.id), eq(smsTemplates.id, input.id)));
        return { success: true };
      }),
  }),
  ...
  syncCalls: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      do {
        ...
        for (const call of inbound.data || []) {
          const existing = await db.select().from(callLogs)
            .where(and(eq(callLogs.tenantId, ctx.tenant!.id), eq(callLogs.vocphoneCallId, String(call.id))))
            .limit(1);
          if (existing.length === 0) {
            const phoneToMatch = call.callerid || "";
            const leadId = phoneToMatch ? await findLeadByPhone(phoneToMatch, ctx.tenant!.id) : null;
            const extNum = call.extension ? parseInt(call.extension) : null;
            let extUserName: string | null = null;
            if (extNum) {
              const [extRow] = await db.select().from(vocphoneExtensions).where(and(eq(vocphoneExtensions.tenantId, ctx.tenant!.id), eq(vocphoneExtensions.extension, extNum))).limit(1);
      ...
      const syncKey = `vocphone_last_sync:${ctx.tenant!.id}`;
      await db.insert(globalSettings)
        .values({ key: syncKey, value: JSON.stringify(new Date().toISOString()) })
  ...
  resyncUnlinkedCalls: adminProcedure.mutation(async ({ ctx }) => {
    const db = (await getDb())!;
    ...
    const unlinked = await db
      ...
      .from(callLogs)
      .where(and(eq(callLogs.tenantId, ctx.tenant!.id), sql`${callLogs.leadId} IS NULL`));
    ...
    for (const call of unlinked) {
      ...
      const leadId = await findLeadByPhone(phoneToMatch, ctx.tenant!.id);
      if (leadId) {
        await db.update(callLogs).set({ leadId }).where(and(eq(callLogs.tenantId, ctx.tenant!.id), eq(callLogs.id, call.id)));
        linked++;
      }
    }
  ...
  getCallStats: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [todayCount] = await db
      .select({ total: count() })
      .from(callLogs)
      .where(and(eq(callLogs.tenantId, ctx.tenant!.id), gte(callLogs.createdAt, todayStart)));

    // Average duration (all time)
    const [avgResult] = await db
      .select({ avgDuration: avg(callLogs.duration) })
      .from(callLogs)
      .where(and(eq(callLogs.tenantId, ctx.tenant!.id), sql`${callLogs.duration} > 0`));
    ...
    const extStats = await db
      ...
      .where(and(
        ...
        eq(callLogs.tenantId, ctx.tenant!.id),
    ...
    const [weekCount] = await db
      .select({ total: count() })
      .from(callLogs)
      .where(and(eq(callLogs.tenantId, ctx.tenant!.id), gte(callLogs.createdAt, weekStart)));

    let busiestUserName: string | null = null;
    if (extStats[0]?.extension) {
      const [extRow] = await db.select().from(vocphoneExtensions).where(and(eq(vocphoneExtensions.tenantId, ctx.tenant!.id), eq(vocphoneExtensions.extension, extStats[0].extension))).limit(1);
  ...
  getCallVolume: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const rows = await db
        ...
        .from(callLogs)
        .where(and(eq(callLogs.tenantId, ctx.tenant!.id), gte(callLogs.createdAt, startDate)))
  ...
  updateCallNotes: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.update(callLogs)
        ...
        .where(and(eq(callLogs.tenantId, ctx.tenant!.id), eq(callLogs.id, input.callId)));
  ...
  bulkMarkReviewed: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db.update(callLogs)
        ...
        .where(and(eq(callLogs.tenantId, ctx.tenant!.id), inArray(callLogs.id, input.callIds)));
      return { success: true, count: input.callIds.length };
    }),

  // ─── Get missed call count (for badge) ────────────────────────────────────
  getMissedCallCount: protectedProcedure.query(async ({ ctx }) => {
    const db = (await getDb())!;
  ...
  getLastSyncTimestamp: protectedProcedure.query(async ({ ctx }) => {
    const db = (await getDb())!;
    const [row] = await db
      .select({ value: globalSettings.value })
      .from(globalSettings)
      .where(eq(globalSettings.key, `vocphone_last_sync:${ctx.tenant!.id}`))
      .limit(1);
  ...
  getAllCalls: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const conditions: any[] = [eq(callLogs.tenantId, ctx.tenant!.id)];
  ...
  // ─── Get extensions ───────────────────────────────────────────────────────
  getExtensions: protectedProcedure.query(async ({ ctx }) => {
    const db = (await getDb())!;
    try {
      const apiExts = await vocphone.getExtensions(ctx.tenant!.id);
      // Merge with local user name mappings
      const localExts = await db.select().from(vocphoneExtensions).where(eq(vocphoneExtensions.tenantId, ctx.tenant!.id));
  ...
  getLocalExtensions: adminProcedure.query(async ({ ctx }) => {
    const db = (await getDb())!;
    return await db.select().from(vocphoneExtensions)
      .where(eq(vocphoneExtensions.tenantId, ctx.tenant!.id))
      .orderBy(vocphoneExtensions.extension);
  }),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.id) {
        await db.update(vocphoneExtensions)
          ...
          .where(and(eq(vocphoneExtensions.tenantId, ctx.tenant!.id), eq(vocphoneExtensions.id, input.id)));
        return { success: true };
      } else {
  ...
  deleteExtension: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const db = (await getDb())!;
      await db.delete(vocphoneExtensions)
        .where(and(eq(vocphoneExtensions.tenantId, ctx.tenant!.id), eq(vocphoneExtensions.id, input.id)));
      return { success: true };
    }),
});
